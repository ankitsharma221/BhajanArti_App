...
class GodQuotesPage extends StatefulWidget {
  @override
  _GodQuotesPageState createState() => _GodQuotesPageState();
}

class _GodQuotesPageState extends State<GodQuotesPage>
  ...
  Future<List<ImageModel>> fetchImages(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic> quotes = data['godQuotes'];
        int totalPages = data['totalPages'];

        _hasMoreImages = page < totalPages;

        List<ImageModel> images =
            quotes.map((item) => ImageModel.fromJson(item)).toList();
  ...
  Future<Uint8List> _getCachedOrDownloadImage(
        ...
        children: [
          ClipRRect(
            ...
            child: FutureBuilder<Uint8List>(
              future: _getCachedOrDownloadImage(
                  imageModel.imageUrl, imageModel.imageName),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return AspectRatio(
                    aspectRatio: 1,
